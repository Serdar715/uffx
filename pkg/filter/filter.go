...
func NewPerDomainFilter(globfilters map[string]ffuf.FilterProvider) *PerDomainFilter {
	// Deep copy to prevent shared map reference
	filtersCopy := make(map[string]ffuf.FilterProvider, len(globfilters))
	for k, v := range globfilters {
		filtersCopy[k] = v
	}
	return &PerDomainFilter{IsCalibrated: false, Filters: filtersCopy}
}
...
func (f *MatcherManager) GetFilters() map[string]ffuf.FilterProvider {
	f.Mutex.Lock()
	defer f.Mutex.Unlock()
	copy := make(map[string]ffuf.FilterProvider, len(f.Filters))
...
func (f *MatcherManager) GetMatchers() map[string]ffuf.FilterProvider {
	f.Mutex.Lock()
	defer f.Mutex.Unlock()
	copy := make(map[string]ffuf.FilterProvider, len(f.Matchers))
...
func (f *MatcherManager) FiltersForDomain(domain string) map[string]ffuf.FilterProvider {
	f.Mutex.Lock()
	defer f.Mutex.Unlock()
	var source map[string]ffuf.FilterProvider
	if f.PerDomainFilters[domain] == nil {
		source = f.Filters
	} else {
		source = f.PerDomainFilters[domain].Filters
	}
	copy := make(map[string]ffuf.FilterProvider, len(source))
