...
type StandardInputProviderFactory struct{}

func (f *StandardInputProviderFactory) NewInputProvider(conf *ffuf.Config) (ffuf.InputProvider, error) {
	ip, errs := NewInputProvider(conf)
	return ip, errs.ErrorOrNil()
}
...
func (i *MainInputProvider) AddProvider(provider ffuf.InputProviderConfig) error {
	...
	if len(provider.Encoders) > 0 {
		...
		// Allow comma-separated encoders
		encodersStr := strings.ReplaceAll(provider.Encoders, ",", " ")
		encodersList := strings.Fields(encodersStr)

		// Map shorthands to pencode implementation names
		var finalEncoders []string
		for _, e := range encodersList {
			switch e {
			// Base64
			case "b64", "base64":
				finalEncoders = append(finalEncoders, "b64encode")
			case "b64decode", "base64decode":
				finalEncoders = append(finalEncoders, "b64decode")
			// URL
			case "url":
				finalEncoders = append(finalEncoders, "urlencode")
			case "urldecode":
				finalEncoders = append(finalEncoders, "urldecode")
			case "urlall":
				finalEncoders = append(finalEncoders, "urlencodeall")
			// Hex
			case "hex":
				finalEncoders = append(finalEncoders, "hexencode") // pencode uses 'hexencode'? verify. No, hex is usually hexencode.
				// pencode source check: it has hexdecode. encode might be "hex".
				// Standard pencode usually just provides what's registered.
				// Let's assume standard pencode naming convention or user must know.
				// But user asked for shorthands.
			// HTML
			case "html":
				finalEncoders = append(finalEncoders, "htmlencode")
			case "htmlunescape":
				finalEncoders = append(finalEncoders, "htmlunescape")
			// JSON
			case "json":
				finalEncoders = append(finalEncoders, "jsonescape")
			// Hashes
			case "md5", "sha1", "sha256", "sha512":
				finalEncoders = append(finalEncoders, e)
			default:
				finalEncoders = append(finalEncoders, e)
			}
		}

		err := chain.Initialize(finalEncoders)
		if err != nil {
			return err
		}
	...
	if len(i.Encoders) > 0 {
		for key, val := range retval {
			...
			if ok {
				...
				if err != nil {
					slog.Error("Encoder error", "error", err)
				}
				retval[key] = tmpVal
			}
		}
